...
async function fetchdata() {
    try {
        ...
        if (!res.ok) {
            throw new Error(data.message || "Failed to load men's styles");
        }

        arr = Array.isArray(data) ? data : [];
        if (!arr.length) {
            showMessage("No men's services available right now.");
            return;
        }
        renderCard(arr);
    } catch (err) {
        console.log(err);
        showMessage("Unable to load men's services. Please try again.");
    }
}
